<div>
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div class="flex-1">
            <h1 class="text-2xl font-bold">Редагування заявки</h1>
            <p class="text-base-content/60 text-sm mt-1 font-mono">{{ $order->number }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="lg:col-span-2 flex flex-col gap-4">
                            ...
                            <div class="absolute z-50 w-full mt-1 bg-base-100 border border-base-300 rounded-lg shadow-lg">
                                @forelse($this->clientResults as $client)
                                    <button wire:click="selectClient({{ $client->id }})"
                                            class="w-full text-left px-4 py-3 hover:bg-base-200 flex justify-between">
                                        <span class="font-medium text-sm">{{ $client->name }}</span>
                                        <span class="text-xs text-base-content/60">{{ $client->phone }}</span>
                                    </button>
                                @empty
                                    <div class="px-4 py-3 text-sm text-base-content/60">Не знайдено</div>
                                @endforelse
                            </div>
                            @endif
                        </div>
                        @error('clientId')<span class="text-error text-xs">{{ $message }}</span>@enderror
                    @endif
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <div class="form-control">
                            ...
                            <select wire:model.live="deviceTypeId" class="select select-bordered select-sm">
                                ...
                                @foreach($this->deviceTypes as $dt)
                            ...
                            @if(!$showNewModel)
                                <div class="relative">
                                    <input wire:model.live.debounce.300ms="modelSearch"
                                           ...
                                           placeholder="{{ $brandId ? 'Пошук моделі...' : 'Спочатку бренд' }}"
                                           @disabled(!$brandId)
                                           autocomplete="off"
                                        ...
                                        @empty
                                            <div class="px-3 py-2 text-sm text-base-content/60">
                                                ...
                                                <button wire:click="$set('showNewModel', true); $set('showModelDropdown', false)"
                                                        class="link link-primary ml-1">Додати</button>
                                            </div>
                                        @endforelse
            ...
            <div class="card bg-base-100 shadow-sm">
                <div class="card-body">
                    <h2 class="card-title text-base">Деталі заявки</h2>
                    ...
                    <div class="form-control">
                        <label class="label"><span class="label-text">Висновок інженера</span></label>
                        <textarea wire:model="diagnosis"
                                  class="textarea textarea-bordered"
                                  rows="2"
                                  placeholder="Технічний висновок після діагностики..."></textarea>
                    </div>
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="card bg-base-100 shadow-sm">
                <div class="card-body">
                    ...
                    <div class="form-control">
                        ...
                        <input wire:model="prepayment" type="number" min="0"
            ...
            <div class="card bg-base-100 shadow-sm">
                <div class="card-body gap-2">
                    ...
                    <a href="{{ route('orders.show', $order) }}" class="btn btn-ghost w-full">
                        Скасувати
                    </a>
                </div>
            </div>
